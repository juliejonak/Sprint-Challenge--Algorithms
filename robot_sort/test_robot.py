...
class Test(unittest.TestCase):
    ...
    def test_stretch_times(self):
        robot = SortingRobot(self.small_list)
        robot.sort()
        print(f'Small list time: {robot._time}')
        print(f'Time to beat: 110. \n')
        self.assertLess(robot._time, 110)

        robot = SortingRobot(self.medium_list)
        robot.sort()
        print(f'Medium list time: {robot._time}')
        print(f'Time to beat: 1948. \n')
        self.assertLess(robot._time, 1948)

        robot = SortingRobot(self.large_list)
        robot.sort()
        print(f'Large list time: {robot._time}')
        print(f'Time to beat: 27513. \n')
        self.assertLess(robot._time, 27513)

        robot = SortingRobot(self.large_varied_list)
        robot.sort()
        print(f'Large varied list time: {robot._time}')
        print(f'Time to beat: 28308. \n')
